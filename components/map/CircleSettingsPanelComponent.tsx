...
export default function CircleSettingsPanelComponent({
    ...
    const handleLatitudeChange = (value: string) => {
        ...
        if (isNaN(numValue)) {
            setStatusMessageLatitude("Latitude must be a valid number");
            return;
        }

        if (numValue > 90) {
            setStatusMessageLatitude("Latitude must be between -90 and 90");
            return;
        }
    ...
    const handleLongitudeChange = (value: string) => {
        ...
        const numValue = Number(value);
        if (isNaN(numValue)) {
            setStatusMessageLongitude("Longitude must be a valid number");
            return;
        }

        if (numValue > 180) {
            setStatusMessageLongitude("Longitude must be between -180 and 180");
            return;
        }
    ...
    return (
        <div className="relative">
            ...
            >
                <span className="break-words text-center hidden md:block">Create Circle</span>
            ...
            {isOpen && buttonPosition && createPortal(
                <div
                    ...
                    style={{
                        ...
                        bottom: window.innerWidth >= 768 ? `${window.innerHeight - buttonPosition.top + 10}px` : 'auto',
                        right: window.innerWidth >= 768 ? `${buttonPosition.right + 40}px` : undefined,
                ...
                >
                    <div className="bg-gradient-to-br from-brand-orange to-brand-orange-dark py-5 px-6 flex justify-between items-center">
                        <div className="flex items-center gap-3">
                            ...
                            <h3 className="m-0 text-lg font-bold text-white tracking-wide">Create new circle</h3>
                        </div>
                        <button
                            type="button"
                            onClick={handleToggle}
                    ...
                    <div className="p-6">
                        <div className="flex flex-col md:flex-row gap-4 mb-4">
                            <div className="flex-1 flex flex-col mb-0">
                                <label htmlFor="latitude" className="text-sm mb-1.5 flex justify-between text-gray-900 dark:text-gray-100 font-semibold">Latitude:</label>
                                <input
                                    id="latitude"
                                    type="number"
                                    value={latitudeInput}
                            ...
                            <div className="flex-1 flex flex-col mb-0">
                                <label htmlFor="longitude" className="text-sm mb-1.5 flex justify-between text-gray-900 dark:text-gray-100 font-semibold">Longitude:</label>
                                <input
                                    id="longitude"
                                    type="number"
                        ...
                        <div className="flex flex-col md:flex-row gap-2.5 md:items-center mb-2.5">
                            <div className="flex-1 flex flex-col mb-0">
                                <label htmlFor="radius" className="text-sm mb-1.5 flex justify-between text-gray-900 dark:text-gray-100 font-semibold">Radius (m):</label>
                                <input
                                    id="radius"
                                    type="number"
                                    value={radius}
