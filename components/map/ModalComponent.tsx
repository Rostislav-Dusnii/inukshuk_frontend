                    ...
                    {isEnhancedMode ? (
                        ...
                        <div className="p-6">
                            ...
                            <div className="flex justify-center mb-4">
                                <div className={`${typeStyles.iconBg} w-16 h-16 rounded-full flex items-center justify-center`}>
                            ...
                            {/* Title */}
                            {title && (
                                <h2 id="modal-title" className={`text-2xl font-bold text-center mb-3 ${typeStyles.titleColor}`}>
                                    {title}
                                </h2>
                            )}
                            ...
                            {/* Message */}
                            {message && (
                                <p id="modal-description" className="text-gray-600 dark:text-gray-400 text-center mb-6 leading-relaxed">
                                    {message}
                                </p>
                            )}
                            ...
                            {/* Custom buttons */}
                            {buttons.length > 0 && (
                                <div className={`flex gap-3 ${buttons.length === 1 ? 'justify-center' : 'justify-stretch'}`}>
                                    {buttons.map((button, index) => (
                                        <button
                                            key={index}
                                            onClick={button.onClick}
                                            className={`flex-1 px-6 py-3 rounded-lg font-semibold text-sm transition-all focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-brand-orange ${getButtonStyles(button.variant)}`}
                                        >
                                            {button.label}
                                        </button>
                                    ))}
                        ...
                        <>
                            ...
                            {title && (
                                <div className="flex items-center justify-between p-4 border-b-2 border-brand-orange/30 dark:border-brand-orange/50 w-full bg-gradient-to-r from-brand-orange/10 to-brand-green/10 dark:from-brand-orange/20 dark:to-brand-green/20">
                                    <h2 id="modal-title" className="text-4xl text-center font-semibold p-0 m-0 text-brand-green dark:text-brand-green-light w-full">
                                        {title}
                                    </h2>
                                </div>
                            )}
                            ...
                            {showFooterClose && (
                                <div className="bt-4 pb-3 pr-2 pl-2 border-t-2 border-brand-orange/30 dark:border-brand-orange/50 w-full bg-gradient-to-r from-brand-orange/5 to-brand-green/5 dark:from-brand-orange/15 dark:to-brand-green/15">
                                    <button
                                        ...
                                        aria-label="Close modal"
                                    >
                                        <p className="text-2xl font-semibold">CLOSE</p>
                                    </button>
                                </div>
                            )}
                        </>
